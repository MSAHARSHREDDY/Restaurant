  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
          ...
          {(['today', 'monthly', 'quarterly', 'yearly'] as const).map(tf => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg text-xs font-mono font-bold uppercase transition-all cursor-pointer ${
                timeframe === tf 
                  ? 'bg-gold-500 text-dark-950 shadow-md font-bold' 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-dark-900 border border-white/5 rounded-xl p-5 space-y-1.5">
      ...
      <div className="bg-dark-900 border border-white/10 rounded-2xl p-6 shadow-2xl space-y-6">
          ...
          {dynamicData.chartData.map((bar, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center group relative z-10">
                ...
                {/* Tooltip */}
                <div className="absolute bottom-full mb-2 bg-dark-950 border border-gold-500/30 px-2.5 py-1.5 rounded-xl text-[10px] text-white font-mono opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none text-center shadow-xl z-20 whitespace-nowrap">
                ...
                {/* The Bar */}
                <div 
                  className="w-full max-w-[48px] bg-gradient-to-t from-gold-600 to-amber-400 rounded-t-lg transition-all duration-500 hover:brightness-110 active:brightness-90 hover:shadow-[0_0_15px_rgba(234,179,8,0.25)] flex items-end justify-center pb-2 text-[9px] font-mono font-bold text-dark-950"
      ...
      <div className="bg-dark-900 border border-white/10 rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-white/5 max-h-[500px] overflow-y-auto">
          {dynamicData.transactions.map((txn, index) => (
            <div key={txn._id || index} className="px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center hover:bg-white/3 transition-colors gap-2">
			  <div>
