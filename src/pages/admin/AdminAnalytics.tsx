...
export function AdminAnalytics() {
  ...
  const stats = React.useMemo(() => {
    ...
    const funnel = [
      ...
      { stage: "Viewed Flight Menu", cnt: Math.round(usersCount * 0.9 + totalOrders), loss: `${Math.round(((usersCount * 0.9 + totalOrders)/(usersCount || 1)) * 100)}%` },
    ...
    return {
      ...
      feedbackSummary,
      funnel,
      activeFlightsCount: Math.max(1, Math.round(totalOrders * 0.7 + 3)),
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-gray-400 space-y-4">
        ...
        <p className="font-mono text-sm tracking-widest uppercase">Analyzing Passenger Metrics & Flight Logs...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      {/* Page Header */}
      <div className="bg-gradient-to-r from-dark-900 to-dark-950 border border-white/10 rounded-2xl p-6 md:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-xl">
        <div>
        ...
        <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 shrink-0">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-xs font-mono transition-all uppercase cursor-pointer ${activeTab === 'cuisine' ? 'bg-gold-500 text-dark-950 font-bold' : 'text-gray-400 hover:text-white'}`}
          >
            Cuisine
          </button>
          <button 
            onClick={() => setActiveTab('ratings')} 
            className={`px-4 py-2 rounded-lg text-xs font-mono transition-all uppercase cursor-pointer ${activeTab === 'ratings' ? 'bg-gold-500 text-dark-950 font-bold' : 'text-gray-400 hover:text-white'}`}
          >
            Feedback
          </button>
          <button 
            onClick={() => setActiveTab('engagement')} 
            className={`px-4 py-2 rounded-lg text-xs font-mono transition-all uppercase cursor-pointer ${activeTab === 'engagement' ? 'bg-gold-500 text-dark-950 font-bold' : 'text-gray-400 hover:text-white'}`}
          >
            Engagement
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-dark-900 border border-white/5 rounded-2xl p-6 flex flex-col justify-between shadow-lg">
      ...
      {activeTab === 'cuisine' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 bg-dark-900 border border-white/10 rounded-2xl p-6 shadow-xl space-y-6">
            <div>
      ...
      {activeTab === 'ratings' && (
        <div className="bg-dark-900 border border-white/10 rounded-2xl p-6 shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-4 space-y-4 bg-white/3 border border-white/5 p-4 rounded-xl">
      ...
      {activeTab === 'engagement' && (
        <div className="bg-dark-900 border border-white/10 rounded-2xl p-6 shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="p-4 border border-white/5 rounded-xl bg-white/3 space-y-2">
              <div className="flex justify-between text-xs text-gray-400 font-mono">
                <span>ACTIVE BOOKINGS</span>
                <span className="text-emerald-400 font-bold">ONLINE</span>
              </div>
              <p className="text-2xl font-bold text-white font-mono">{stats.activeFlightsCount} Flights</p>
